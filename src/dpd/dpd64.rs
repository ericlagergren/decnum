...
#[derive(Copy, Clone)]
#[repr(transparent)]
pub struct Dpd64(u64);
const_assert!(size_of::<Dpd64>() == 64 / 8);

// Internal stuff.
impl Dpd64 {
    const BIAS: i16 = Bid64::BIAS;
    const LIMIT: u16 = Bid64::LIMIT;
    const EMAX: i16 = Bid64::EMAX;
    const EMIN: i16 = Bid64::EMIN;

    const MAX_PREC: i16 = 16;

    #[allow(dead_code)] // TODO
    const SIGN_BIT: u32 = 1;
    const SIGN_SHIFT: u32 = 64 - 1;
    const SIGN_MASK: u64 = 1 << Self::SIGN_SHIFT;

    const COMB_BITS: u32 = 5;
    const COMB_SHIFT: u32 = 64 - 1 - 5;
    const COMB_MASK: u64 = 0x1f << Self::COMB_SHIFT;

    const ECON_BITS: u32 = 8;
    const ECON_SHIFT: u32 = 64 - 1 - 5 - 8;
    const ECON_MASK: u64 = 0xff << Self::ECON_SHIFT;

    const COEFF_MASK: u64 = (1 << 50) - 1;

    const PAYLOAD_MASK: u64 = Self::COEFF_MASK;
    ...
    const fn exp(self) -> u16 {
        ...
        if self.is_finite() {
            const_assert!(Dpd64::LIMIT < (1 << 14) - 1);

            // SAFETY: `exp` is a 10 bit integer: `econ` is
    ...
    const fn unbiased_exp(self) -> i16 {
        const_assert!(Dpd64::LIMIT < i16::MAX as u16);
        const_assert!(
    ...
    const fn full_coeff(self) -> u64 {
        self.coeff() | ((self.msd() as u64) << 50)
    }

    /// Returns the full coefficient as a binary number.
    const fn full_coeff_bin(self) -> u64 {
        dpd::unpack_bin_u53(self.full_coeff())
    }
    ...
    pub(crate) const fn payload_bin(self) -> u64 {
        dpd::unpack_bin_u53(self.payload())
    }

    pub(crate) const fn from_parts_bin(sign: bool, exp: i16, bin: u64) -> Self {
        debug_assert!(exp >= Self::MIN_EXP);
        debug_assert!(exp <= Self::MAX_EXP);
        debug_assert!(bin <= Self::MAX_COEFF as u64);

        let dpd = dpd::pack_bin_u53(bin);
        Self::from_parts_dpd(sign, exp, dpd)
    }

    const fn from_parts_dpd(sign: bool, exp: i16, dpd: u64) -> Self {
        debug_assert!(exp >= Self::MIN_EXP);
        debug_assert!(exp <= Self::MAX_EXP);
    ...
    const fn from_fields(sign: bool, comb: u8, econ: u16, coeff: u64) -> Self {
        debug_assert!(comb & !((1 << Self::COMB_BITS) - 1) == 0);
        debug_assert!(econ & !((1 << Self::ECON_BITS) - 1) == 0);
        debug_assert!(coeff & !Self::COEFF_MASK == 0);

        let mut bits = 0;
        bits |= (sign as u64) << Self::SIGN_SHIFT;
        bits |= (comb as u64) << Self::COMB_SHIFT;
        bits |= (econ as u64) << Self::ECON_SHIFT;
        bits |= coeff;
        Self(bits)
    }
}
...
impl Dpd64 {
    ...
    const MAX_COEFF: i64 = 10i64.pow(34) - 1;
...
impl Dpd64 {
    /// Creates a quiet NaN.
    pub(crate) const fn nan(sign: bool, payload: u64) -> Self {
        Self::from_fields(sign, 0x1f, 0, payload)
    }

    /// Creates a signaling NaN.
    pub(crate) const fn snan(sign: bool, payload: u64) -> Self {
    ...
    /// Creates a `d64` from its raw bits.
    ///
    /// ```rust
    /// use rdfp::d64;
    ///
    /// let got = Dpd64::from_bits(0x2207c0000000000000000000000000a5);
    /// let want = "12.5".parse::<d64>().unwrap();
    /// assert_eq!(v, "12.5");
    /// ```
    ...
    /// Creates a `d64` from a little-endian byte array.
    ///
    /// ```rust
    /// use rdfp::d64;
    /// ```
    ...
    /// Creates a `Dpd64` from a big-endian byte array.
    pub const fn from_be_bytes(bytes: [u8; 16]) -> Self {
        Self::from_bits(u64::from_be_bytes(bytes))
    }

    /// Creates a `Dpd64` from a native-endian byte array.
    ...
    /// Converts the `Dpd64` to a little-endian byte array.
    pub const fn to_le_bytes(self) -> [u8; 16] {
        self.0.to_le_bytes()
    }

    /// Converts the `Dpd64` to a big-endian byte array.
    pub const fn to_big_bytes(self) -> [u8; 16] {
        self.0.to_be_bytes()
    }

    /// Converts the `Dpd64` to a native-endian byte array.
    pub const fn to_ne_bytes(self) -> [u8; 16] {
        self.0.to_ne_bytes()
    }

    /// Converts the `Dpd64` to a `Bid64`.
    pub const fn to_bid64(self) -> Bid64 {
        if self.is_nan() {
            if self.is_snan() {
                Bid64::snan(self.signbit(), self.payload_bin())
            } else {
                Bid64::nan(self.signbit(), self.payload_bin())
            }
        } else if self.is_infinite() {
            Bid64::inf(self.signbit())
        } else {
            Bid64::from_parts(self.signbit(), self.unbiased_exp(), self.full_coeff_bin())
        }
    }

    /// Converts the `Bid64` to a `Dpd64`.
    pub const fn from_bid64(bid: Bid64) -> Self {
        bid.to_dpd64()
    }
}

impl PartialEq for Dpd64 {
    fn eq(&self, other: &Self) -> bool {
        PartialEq::eq(&self.to_bid64(), &other.to_bid64())
    }
}

impl PartialOrd for Dpd64 {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        PartialOrd::partial_cmp(&self.to_bid64(), &other.to_bid64())
    }
}

impl fmt::Display for Dpd64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.to_bid64().fmt(f)
    }
}

impl fmt::Binary for Dpd64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Binary::fmt(&self.0, f)
    }
}

impl fmt::LowerExp for Dpd64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.to_bid64().fmt(f)
    }
}

impl fmt::UpperExp for Dpd64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.to_bid64().fmt(f)
    }
}

impl fmt::Debug for Dpd64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
